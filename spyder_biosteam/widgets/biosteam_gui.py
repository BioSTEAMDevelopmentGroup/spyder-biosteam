...
from biosteam._digraph import (get_all_connections,
...
class FlowsheetWidget(QWidget):
    ...
    def get_digraph(self, system, flowsheet, connections):
        kind = self.kind
        if kind == 'thorough':
            digraph = digraph_from_units_and_connections(flowsheet.units,
                                                         connections)
        elif kind == 'surface':
            digraph = surface_digraph(system.path)
            streams = flowsheet.stream.get_values()
            units = flowsheet.unit.get_values()
            ...
            update_digraph_from_units_and_connections(other_units,
                                                      other_connections)
        elif kind == 'minimal':
